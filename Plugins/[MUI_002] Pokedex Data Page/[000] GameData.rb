...
module GameData
  class Evolution
    ...
    def description(species, evo, param = nil, full = true, form = false, c = [])
      ...
      full_name = c[1] + full_name + c[4] if !c.empty?
      #-------------------------------------------------------------------------
      ...
      when Symbol
        ...
        param = c[2] + par + c[4] if !c.empty?
        param_name = _INTL("{1}{2}", prefix, param)
        param = c[2] + par2 + c[4] if !c.empty? && par2
      ...
      when Integer
        ...
        if param_name
          param_name = c[2] + param_name + c[4] if !c.empty?
          param_name2 = c[2] + param_name2 + c[4] if param_name2 && !c.empty?
        else
        ...
        case param
        when "MossRock"
          location = (c.empty?) ? "Moss Rock" : c[2] + "Moss Rock" + c[4]
          param_name = _INTL("near a {1}", location)
        when "IceRock"
          location = (c.empty?) ? "Ice Rock" : c[2] + "Ice Rock" + c[4]
          param_name = _INTL("near an {1}", location)
        when "Magnetic"
          location = (c.empty?) ? "magnetic area" : c[2] + "magnetic area" + c[4]
          param_name = _INTL("in a {1}", location)
        else
          location = (c.empty?) ? "special area" : c[2] + "special area" + c[4]
          param_name = _INTL("in a {1}", location)
        end
      end
      #-------------------------------------------------------------------------

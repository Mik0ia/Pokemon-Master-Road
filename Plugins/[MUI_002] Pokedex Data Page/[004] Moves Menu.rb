...
class PokemonPokedexInfo_Scene
  ...
  def pbDrawMoveList
    ...
    when 2 then title = _INTL("Egg Moves")
    ...
    4.times do |i|
      ...
      textpos.push([moveName, 58, yPos, :left, base, shadow, :outline],
                   [_INTL("PP"), 144, yPos + 30, :left, base, shadow],
                   [movePP > 0 ? movePP.to_s : "??", 230, yPos + 30, :right, base, shadow])
      case @moveListIndex
      #-------------------------------------------------------------------------
      when 0  # Level-up moves
        if moveLevel == 0
    ...
    if !@moveCommands.empty?
      ...
      drawTextEx(overlay, 272, 216, 230, 5, selMoveData.description, base, shadow)
    end

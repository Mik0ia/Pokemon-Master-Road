...
class PokemonPokedexInfo_Scene
  ...
  def pbDrawDataNotes(cursor = nil)
    ...
    when :ability
      pbDrawImagePositions(overlay, [[path, 242, 240, 0, 244, 120, 44]])
      text = t[4] + "Abilities\n"
    ...
    when :moves
      pbDrawImagePositions(overlay, [[path, 370, 240, 0, 244, 120, 44]])
      text = t[4] + "Moves\n"
      if $player.owned?(@species)
  ...
  def pbDataTextGeneral(path, species, overlay, inMenu = false)
    ...
    if owned
      ...
      text << gender + t[4] + "." if gender != ""
      pbDrawTextPositions(overlay, [
        [_INTL("View Compatible"), Graphics.width - 34, 292, :right, Color.new(120, 184, 232), Color.new(0, 112, 248)]
  ...
  def pbDataTextItems(path, species, overlay)
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 224, 162, 432, 206, 76, 74]])
    owned = $player.owned?(@species)
    text = ""
    if owned
      @data_hash[:item].keys.each_with_index do |r, a|
        ...
        @data_hash[:item][r].each_with_index do |item, i|
          text << t[1] + GameData::Item.get(item).name + t[4]
      ...
      if nil_or_empty?(text)
        ...
        pbDrawTextPositions(overlay, [
          [_INTL("View Details"), Graphics.width - 34, 292, :right, Color.new(120, 184, 232), Color.new(0, 112, 248)]
        ])
      end
    else
      text << "Unknown."
  ...
  def pbDataTextHabitat(path, species, overlay, s2 = nil)
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 440, 162, 432, 206, 76, 74]])
    owned = $player.owned?(@species)
    text = t[4] + "Habitat\n"
    if owned
      ...
      case habitat.id
      when :Grassland    then text << "roaming within wide open " + nt + _INTL("{1}", name) + t[4] + " areas."
      when :Forest       then text << "within densely wooded areas, such as a " + nt + _INTL("{1}", name) + t[4] + "."
      when :WatersEdge   then text << "within areas near the " + nt + _INTL("{1}", name) + t[4] + "."
      when :Sea          then text << "roaming above or below bodies of water, such as the " + nt + _INTL("{1}", name) + t[4] + "."
      when :Cave         then text << "within dark and secluded areas, such as a " + nt + _INTL("{1}", name) + t[4] + "."
      when :Mountain     then text << "up high, scaling the sides of " + nt + _INTL("{1}", name) + t[4] + " ranges."
      when :RoughTerrain then text << "within harsher locales with " + nt + _INTL("{1}", name) + t[4] + "."
      when :Urban        then text << "near man-made structures or within " + nt + _INTL("{1}", name) + t[4] + " areas."
      when :Rare         then text << "only in very " + nt + _INTL("{1}", name) + t[4] + " situations or locations."
  ...
  def pbDataTextShape(path, species, overlay, s2 = nil)
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 368, 162, 432, 206, 76, 74]])
    text = t[4] + "Morphology\n"
    color = GameData::BodyColor.get(species.color)
    nt = (s2 && s2[0] == color.id) ? t[2] : t[1]
    name = color.name.downcase
    text << "This species is primarily " + nt + _INTL("{1}", name) + t[4] + " in color, and its body "
    ...
    when :Head, :HeadArms, :HeadBase, :HeadLegs
      text << "shape is just a " + nt + _INTL("{1}", name) + t[4] + "."
    when :Bipedal, :BipedalTail, :Quadruped, :Multiped, :MultiBody, :MultiWinged, :Winged, :Serpentine
      text << "has a " + nt + _INTL("{1}", name) + t[4] + " shape."
    when :Insectoid
      text << "has an " + nt + _INTL("{1}", name) + t[4] + " shape."
    when :Finned
      text << "is " + nt + _INTL("{1}", name) + t[4] + " and shaped for swimming."
    else
  ...
  def pbDataTextEggGroup(path, species, overlay, s2 = nil)
    ...
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 296, 162, 432, 206, 76, 74]])
    owned = $player.owned?(@species)
    text = t[4] + "Breeding\n"
    if owned
      text << "This species "
      ...
      if groups.include?(:None)
        ...
        text << "is genderless, and may only breed with species in the " + t[1] + "#{name}" + t[4] + " group."
      elsif groups.include?(:Ditto)
        data = GameData::EggGroup.get(:Ditto)
        name = (Settings::ALT_EGG_GROUP_NAMES) ? data.alt_name : data.name
        text << "is in the " + t[1] + "#{name}" + t[4] + " group, and may breed with species in all other groups."
      elsif groups.include?(:Undiscovered) || groups.empty?
        data = GameData::EggGroup.get(:Undiscovered)
        name = (Settings::ALT_EGG_GROUP_NAMES) ? data.alt_name : data.name
        text << "is in the " + t[1] + "#{name}" + t[4] + " group, and is incapable of breeding."
      else
    ...
    if nil_or_empty?(text)
          ...
          family_ids.each_with_index do |fam, i|  
            ...
            if i < family_ids.length - 1
              if fam == GameData::Species.get(family_ids[i + 1]).get_previous_species
                text << t[4] + "=> "  # Shows evolution pathway.
              else
                text << t[4] + ", "   # Shows a new pathway.
              end				
            end
          end
        end
      ...
      #-------------------------------------------------------------------------
        ...
        if species.species == :ALCREMIE
          name = t[1] + "#{prevo_data.name}" + t[4]
          text << "Use various " + t[2] + "Sweets" + t[4] + " on #{name}."
        else
          index = 0
      ...
      if !inMenu && $player.owned?(@species) && !@data_hash[:family].empty?
        pbDrawTextPositions(overlay, [
          [_INTL("View Family"), Graphics.width - 34, 292, :right, Color.new(120, 184, 232), Color.new(0, 112, 248)]
        ])
      end
    end
    return text
  ...
  def pbDataTextItemSource(path, species, overlay, item)
    ...
    text << t[4] + " item that may be held by this species."
    return text
  end
  ...
  def pbDataTextAbilitySource(path, species, overlay, ability)
    ...
    text << t[4] + " ability for this species."
    return text
  end
  ...
  def pbDataTextMoveSource(path, species, overlay)
    ...
    text = t[2] + "#{moveName}\n"
    text << t[4] + "Learned by this species "
    methods = []
    #---------------------------------------------------------------------------
    ...
    species.moves.each do |m|
      next if m[1] != moveID
      case m[0]
      when -1 then method = "through " + t[1] + "move relearning" + t[4]  # Gen 9 move relearning.
      when 0  then method = "upon " + t[1] + "evolution" + t[4]           # Evolution move.
      else         method = "at " + t[1] + "level #{m[0]}" + t[4]         # Level-up move.
      end
      methods.push(method)
      break	  
    end
    ...
    if species.tutor_moves.include?(moveID)
      method = "visiting a " + t[1] + "move tutor" + t[4]
